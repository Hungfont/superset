...
type Slice struct {
	...
	LastSavedByFK uint      `gorm:"column:last_saved_by_fk" json:"last_saved_by_fk"`
...
type DashboardSlice struct {
	ID          uint `gorm:"primaryKey;autoIncrement" json:"id"`
	DashboardID uint `gorm:"column:dashboard_id;not null;uniqueIndex:idx_dash_slice,priority:1" json:"dashboard_id"`
	SliceID     uint `gorm:"column:slice_id;not null;uniqueIndex:idx_dash_slice,priority:2" json:"slice_id"`
}
...
type SliceUser struct {
	ID      uint `gorm:"primaryKey;autoIncrement" json:"id"`
	SliceID uint `gorm:"column:slice_id;not null" json:"slice_id"`
	UserID  uint `gorm:"column:user_id;not null" json:"user_id"`
}
